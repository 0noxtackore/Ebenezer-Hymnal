...
export default function ChorusRapidos() {
  const { key } = useParams()
  ...
  const coros = useMemo(() => {
    return hymns
      .filter((h) => {
        const matchCat = strip(h.category) === strip(CATEGORY)
  ...
  const compilationId = strip(CATEGORY) + '#' + decodedKey
  ...
  const share = async () => {
    ...
    try {
      ...
      doc.text(CATEGORY.toUpperCase(), pw / 2, y, { align: 'center' })
      y += 8
      ...
      const slug = `${CATEGORY}-${decodedKey}`.toLowerCase().replace(/\s+/g, '-')
      const fileName = `${slug}-coros.pdf`
      const blob = doc.output('blob')
      const file = new File([blob], fileName, { type: 'application/pdf' })

      if (navigator.share) {
        await navigator.share({ files: [file], title: `${decodedKey} - ${CATEGORY}` })
      } else {
      ...
      <div className="compilation-header">
        ...
        <div style={{ flex: 1 }}>
          ...
          <div className="compilation-sub">
            {coros.length} coro{coros.length !== 1 ? 's' : ''} · {CATEGORY}
          </div>
        </div>
