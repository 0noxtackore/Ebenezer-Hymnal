...
self.addEventListener('fetch', (e) => {
  ...
  if (url.hostname.includes('firebasestorage.googleapis.com') || url.hostname.includes('firebaseio.com') || url.hostname.includes('googleapis.com')) {
    e.respondWith(
      ...
      caches.open(STATIC_CACHE).then((cache) =>
        cache.match(e.request).then((cached) => {
          return fetch(e.request).then((res) => {
            if (res.ok) cache.put(e.request, res.clone())
            return res
          }).catch(() => cached || cache.match('/index.html').then((c) => c || offlineFallback()))
        })
      )
    )
    return
